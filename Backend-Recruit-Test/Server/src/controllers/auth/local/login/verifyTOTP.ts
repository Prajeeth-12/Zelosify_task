...
export const verifyTOTP = async (
  ...
  try {
    ...
    try {
      ...
      // Remove sensitive data and flatten tenant for the frontend
      const { totpSecret, tenant, ...userData } = user;

      // Send the final response with flattened tenantId + companyName
      res.json({
      ...
      logger.error("Error exchanging refresh token", {
