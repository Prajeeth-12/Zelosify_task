...
export function middleware(request) {
  ...
  if (isPublicPath && isAuthenticated) {
    // If role is null/unknown, don't redirect — let the user stay on the page
    // (avoids infinite redirect loops when role hasn't been assigned in Keycloak yet)
    if (!userRole) {
      console.log("User authenticated but role is null — allowing access to public page");
      return response;
    }
    ...
    switch (userRole) {
      case "ADMIN":
        console.log(`Redirecting ADMIN to /user`);
      ...
      case "FINANCE_MANAGER":
        console.log(`Redirecting FINANCE_MANAGER to /user`);
        return NextResponse.redirect(new URL("/user", request.url));

      case "PROCUREMENT_MANAGER":
        console.log(`Redirecting PROCUREMENT_MANAGER to /user`);
        return NextResponse.redirect(new URL("/user", request.url));

      case "RESOURCE_MANAGER":
        console.log(`Redirecting RESOURCE_MANAGER to /user`);
        return NextResponse.redirect(new URL("/user", request.url));

      default:
        // Fallback: allow access instead of redirecting to avoid loops
        console.log(`Unhandled role (${userRole}) — allowing access`);
        return response;
    }
  }
