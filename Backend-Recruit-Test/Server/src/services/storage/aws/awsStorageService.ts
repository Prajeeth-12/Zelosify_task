...
export class AwsStorageService extends StorageService {
  ...
  async getObjectURL(key: string): Promise<string> {
    try {
      ...
      logger.error("AWS S3: Error generating get URL", {
  ...
  async getObjectStream(key: string): Promise<Readable> {
    try {
      logger.debug("AWS S3: Getting object stream", {
      ...
      logger.error("AWS S3: Error getting object stream", {
        meta: { error: String(error) },
      });
      throw error;
    }
  ...
  async putObject(
    ...
    try {
      ...
      logger.error("AWS S3: Error uploading file", {
        meta: { error: String(error) },
      });
      throw error;
    }
  ...
  async listObjects(prefix: string): Promise<any[]> {
    try {
      ...
      logger.error("AWS S3: Error listing objects", {
        meta: { error: String(error) },
      });
      throw error;
    }
  ...
  async getUploadURL(key: string): Promise<string> {
    try {
      logger.debug("AWS S3: Generating upload URL", {
        meta: {
          bucket: this.bucket,
          key,
      ...
      logger.info("AWS S3: Generated upload URL successfully");
      return url;
    } catch (error) {
      logger.error("AWS S3: Error generating upload URL", {
        meta: { error: String(error) },
      });
      throw error;
    }
