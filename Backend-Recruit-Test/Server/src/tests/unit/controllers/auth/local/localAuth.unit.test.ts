...
import {
  ...
  verifyLogin,
  verifyTOTP,
} from "@/controllers/auth/local/localAuthController";

// Mock the implementation modules
vi.mock(
  "@/controllers/auth/local/register/localRegister",
  () => ({
...
vi.mock("@/controllers/auth/local/login/localLogin", () => ({
  verifyLogin: vi.fn(async (req, res) => res.json({ status: "logged-in" })),
}));
vi.mock("@/controllers/auth/local/login/verifyTOTP", () => ({
  verifyTOTP: vi.fn(async (req, res) => res.json({ status: "totp-verified" })),
}));

// Import the mocked implementation modules after mocking
import * as registerImplModule from "@/controllers/auth/local/register/localRegister";
import * as verifyLoginImplModule from "@/controllers/auth/local/login/localLogin";
import * as verifyTOTPImplModule from "@/controllers/auth/local/login/verifyTOTP";
