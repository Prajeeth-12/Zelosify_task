...
export const logout = asyncHandler(
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!refreshToken) {
        logger.warn("No refresh token found, already logged out");
        res
          .status(400)
      ...
      logger.info("Logging out user", { meta: { userId: user?.id } });
      ...
      if (user && user.provider === "KEYCLOAK") {
        ...
        try {
          ...
          logger.info("Keycloak session invalidated for user");
        } catch (error: any) {
          // Type as 'any' for axios error
          logger.error("Keycloak logout request failed", {
        ...
        logger.info("OAuth user logout: clearing cookies");
      }
      ...
      logger.error("Logout error", { meta: { error: String(error) } });
