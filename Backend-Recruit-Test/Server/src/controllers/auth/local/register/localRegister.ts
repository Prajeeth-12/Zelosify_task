...
export const register = asyncHandler(
  async (req: Request<{}, any, RegisterRequest>, res: Response) => {
    try {
      ...
      if (keycloakUser.id && role) {
        try {
          ...
          if (!roleResponse.data || !roleResponse.data.id) {
            logger.error(`Role "${role}" not found in Keycloak`);
          } else {
            try {
              ...
              logger.info(`Role "${role}" assigned to user ${keycloakUser.id}`, {
                meta: { keycloakUserId: keycloakUser.id, role },
              });
            } catch (error) {
              logger.error(`Error assigning role "${role}" to user ${keycloakUser.id}`, {
                meta: { error: String(error) },
              });
            }
          }
        } catch (error) {
          logger.error(`Failed to fetch role "${role}" from Keycloak`, {
            meta: { error: String(error) },
          });
        }
      }
      ...
      // Set registration_token cookie to trigger TOTP setup flow in the frontend middleware
      res.cookie("registration_token", "pending_totp_setup", {
        httpOnly: false,
        secure: process.env.NODE_ENV === "production",
        sameSite: "strict",
        maxAge: 10 * 60 * 1000, // 10 minutes for TOTP setup
        path: "/",
      });
