  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loginWithCredentials.fulfilled, (state, action) => {
        state.loading = false;
        // For seeded users (bypass TOTP), backend returns { success: true, user: {...} }
        // For normal users, backend returns { message: "Login verified..." } without user
        if (action.payload?.success && action.payload?.user) {
          state.user = action.payload.user;
          saveUserToStorage(action.payload.user);
        }
        // Otherwise don't set user yet — TOTP verification is still needed
      })
